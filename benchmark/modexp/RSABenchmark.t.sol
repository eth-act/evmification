...
import {RSAVerifyMontgomery} from "../../examples/modexp/RSAVerifyMontgomery.sol";
import {RSAVerifyBarrett} from "../../examples/modexp/RSAVerifyBarrett.sol";
import {RSAVerifyPure} from "../../examples/modexp/RSAVerifyPure.sol";
...
/// @notice Thin wrapper for Barrett library calls for gas measurement.
contract RSAVerifyBarrettCaller {
    function verify(
        bytes calldata modulus,
        bytes calldata exponent,
        bytes calldata message,
        ...
        return RSAVerifyBarrett.verify(modulus, exponent, message, signature);
    }
}
...
contract RSABenchmarkTest is Test {
    ...
    function test_rsa2048_verify_barrett() public view {
        bool valid = barrettVerifier.verify(N_2048, E, MSG, SIG_2048);
        require(valid, "Barrett RSA-2048 verification failed");
    }
