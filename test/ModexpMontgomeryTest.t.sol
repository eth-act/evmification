...
contract ModexpMontgomeryTest is Test {
    ...
    function test_montgomery_matches_precompile_2048() public view {
        bytes memory expected = precompile.modexp(BASE_2048, E, N_2048);
        bytes memory actual = montgomery.modexp(BASE_2048, E, N_2048);
        assertEq(actual, expected, "Montgomery 2048 mismatch");
    }

    function test_montgomery_matches_precompile_4096() public view {
        bytes memory expected = precompile.modexp(BASE_4096, E, N_4096);
        bytes memory actual = montgomery.modexp(BASE_4096, E, N_4096);
        assertEq(actual, expected, "Montgomery 4096 mismatch");
    }

    function test_modexp_base_zero_2048() public view {
        bytes memory zeroBase = new bytes(256);
        bytes memory expected = precompile.modexp(zeroBase, E, N_2048);
        bytes memory actual = montgomery.modexp(zeroBase, E, N_2048);
        assertEq(actual, expected, "base=0 mismatch");
    }

    function test_modexp_base_one_2048() public view {
        bytes memory oneBase = new bytes(256);
        oneBase[255] = 0x01;
        bytes memory expected = precompile.modexp(oneBase, E, N_2048);
        bytes memory actual = montgomery.modexp(oneBase, E, N_2048);
        assertEq(actual, expected, "base=1 mismatch");
    }

    function test_modexp_exp_one_2048() public view {
