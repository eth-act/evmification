...
/// @title Blake2bPure
/// @notice BLAKE2b hash using pure Solidity F compression (no precompile).
library Blake2bPure {
    ...
    function hash(bytes memory data, uint64 outlen) internal pure returns (bytes memory digest) {
        require(outlen >= 1 && outlen <= 64, "Blake2bPure: invalid outlen");
        ...
        while (offset + 128 < dataLen) {
            ...
            offset += 128;
            t[0] += 128;
            h = Blake2f.compress(12, h, m, t, false);
        }
        ...
        h = Blake2f.compress(12, h, mFinal, t, true);
